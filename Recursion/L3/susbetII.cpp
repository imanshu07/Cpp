...
void storeSubset(string ans,string original, vector <string>& v, bool flag){
    ...
    if(original.length()==1){
        if(flag==true) storeSubset(ans+ch,original.substr(1),v,true); // draw tree and smjh aajayega
        storeSubset(ans,original.substr(1),v,true);
        return; // taki neeche ki do condition na chale
    }
    
    char dh = original[1];
    if(ch==dh){ // to counter duplicates
        if(flag==true) storeSubset(ans+ch,original.substr(1),v,true);
        storeSubset(ans,original.substr(1),v,false);                   // have to watch video to revies or else you won't understand
    }
    else{ // no duplicate
        if(flag==true) storeSubset(ans+ch,original.substr(1),v,true);
        storeSubset(ans,original.substr(1),v,true);
